...
- **Channel boot health-check timeout.** After a `--channels` launch the supervisor waits for `bun server.ts` to appear before marking the session healthy; if it never does, it kills + respawns. That wait defaults to 60s and is tunable via `CHANNEL_HEALTH_TIMEOUT` (seconds) in the workspace `.env` (`026-channel-watchdog-timeout`) — a hardcoded 20s used to flap slow hosts. It interacts with the crash budget: an override `≥65s` logs a boot warning that 5 failures may no longer fit the 300s window, so the container might not escalate to a restart.
