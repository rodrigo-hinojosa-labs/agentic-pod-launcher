...
# CLAUDE_MD is overridable so host bats tests can exercise the prompt without
# a real /workspace.
refresh_claude_md() {
  local claude_md="${CLAUDE_MD:-/workspace/CLAUDE.md}"
  if [ -f "$claude_md" ] && command -v claude >/dev/null 2>&1; then
    ...
    local update_prompt_file update_prompt refresh_ok
    update_prompt_file=$(mktemp)
    cat > "$update_prompt_file" <<'PROMPT'
Update /workspace/CLAUDE.md to reflect the current workspace.

First scan the file's existing `## ` section headers. Preserve ALL
existing sections verbatim — do not edit or reorder any section that is
already present. An operator-injected section must survive byte-for-byte;
the personality sections from agent.yml (Identity, User, etc.) included.

Only ADD sections that are missing, for:
...
Edit in place with the Edit tool, never Write. Do not touch files other
than CLAUDE.md. If every section above already exists, make no edits and
say so.
PROMPT
...
# Run the wizard unless sourced. Host bats tests source this file with
# WIZARD_CONTAINER_NO_RUN=1 to call refresh_claude_md in isolation without
# triggering the interactive Telegram/PAT/Atlassian flow.
if [ "${WIZARD_CONTAINER_NO_RUN:-0}" != "1" ] && [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  main "$@"
fi
