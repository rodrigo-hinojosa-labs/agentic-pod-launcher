...
**Scale/Scope**: ~15 archivos tocados (5 templates `modules/local-*` — reindex/watch/vault-backup/killswitch/healthcheck —, `mcp-json.tpl`, `next-steps.{en,es}.tpl` ×2, `scripts/lib/qmd_index.sh`, `scripts/lib/local_schedule.sh`, `docker/Dockerfile`, `setup.sh`, `scripts/agentctl`) + ~9 suites de test
...
scripts/
├── agentctl                      # US3: doctor last_status/staleness/watcher-failed/exit codes; acciones manuales locales
├── lib/local_schedule.sh         # US3/D10: señal explícita CRON_FALLBACK (sin cambiar rc/stdout)
...
├── scaffold.bats / schema.bats / deployment-mode.bats / local-schedule.bats / uninstall.bats  # aserciones nuevas en suites existentes
...
- **D8 — Acciones manuales locales**: `agentctl heartbeat qmd-reindex|backup-vault` en modo local ejecutan directamente el script del workspace (`scripts/local/agent-qmd-reindex.sh` / `agent-vault-backup.sh`) como el operador — mismo usuario que `User=` de las units, sin `systemctl start` (evita polkit/sudo). Política de flags (analyze U3): `--dry-run` se pasa SOLO a `backup-vault` (único script que lo soporta, `local-vault-backup.sh.tpl:27-28`); para `qmd-reindex --dry-run` agentctl rechaza con error explícito — jamás un reindex real bajo apariencia de dry-run.
- **D9 — Doctor honesto**: `_local_vault_qmd_doctor` lee `.last_status` de `qmd-index.json` (warn/fail en `error`); staleness del backup reusando `_check_backup_freshness` (umbral 25h, igual docker); observa además `systemctl is-failed` de la unit qmd-watch (warn + exit ≥1 — cierra el tercer escenario de SC-005, analyze G1); `cmd_local_doctor` replica el epílogo de exit codes 0/1/2 de `cmd_doctor`. `_local_vault_qmd_status` agrega `last_run`. El reporte "staged (run --login)" para units staged sin instalar YA existe de 012 (`scripts/agentctl:947-952`) y cumple el mínimo del edge case del spec — 013 lo blinda con aserción (analyze G2), no lo reimplementa.
- **D10 — Fallback de schedule persistente**: mecanismo de señal explícito (analyze U1 — hoy `cron_to_systemd_calendar` retorna SIEMPRE rc 0 y avisa solo por stderr; comparar stdout==default da falso positivo con `*/5`, que convierte exacto al default): `scripts/lib/local_schedule.sh` setea una variable global `CRON_FALLBACK=0|1` al convertir (contrato rc/stdout intacto; source-safe, sin side effects — Principle III). `setup.sh` la consulta tras la conversión: escribe `<ws>/scripts/heartbeat/qmd-schedule.fallback` (original + convertido + timestamp) cuando es 1 y lo borra cuando es 0; el render en modo docker borra el marker incondicionalmente (cubre el mode-switch local→docker sin huérfanos — analyze C1); `status`/`doctor` lo reportan. Derivado puro del regenerate (Principle I).
...
- **D13 — Warning del wizard** (analyze G3): en modo local con `vault.qmd.enabled=true` y respuesta "no" a instalar el servicio, `setup.sh` imprime una advertencia explícita de que el RAG queda sin trigger automático (hoy degrada en silencio). Es un `printf` post-respuesta — NO agrega prompt, así que no gatilla el gotcha wizard_answers/e2e-smoke/schema.
