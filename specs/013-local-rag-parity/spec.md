...
- **RC2 — PATH bajo systemd**: las 3 units de 012 (`modules/local-qmd-reindex.service.tpl:4-8`, `local-qmd-watch.service.tpl:5-19`, `local-vault-backup.service.tpl:4-8`) no fijan PATH; el default de systemd excluye `~/.local/bin` (bunx) y `<ws>/scripts/vendor/bin` (yq v4). Cada tick: `command -v bunx` falla (`qmd_index.sh:84-89`) → exit 0 "exitoso" → el índice jamás se refresca. `yq` inalcanzable rompe además `_qmd_enabled`, `qmd_pkg`, la lectura de `agent.yml` del watcher y el `fork_url` del backup (backup nunca pushea pese a fork configurado). Misma clase de bug que 011 corrigió para la unit de sesión (`remote-control.env.tpl:13`) — no se replicó a los contextos batch.
- **RC3 — Watcher sin env del vault**: `modules/local-qmd-watch.sh.tpl:15-16` no exporta `QMD_VAULT_DIR`/`VAULT_ROOT_OVERRIDE` → `vault_resolve_root` rebasea al default de contenedor `/home/agent/.vault` (inexistente en el host) → el watcher sale al arrancar y con `Restart=always` + start-limit default queda **failed permanente en <35s**. Reindex-on-change local = inexistente.
...
- **Estados operacionales**: `qmd-index.json` (última corrida, resultado, fallback de schedule) y `vault-backup.json` en `<ws>/scripts/heartbeat/` — misma ubicación y schema en ambos modos.
