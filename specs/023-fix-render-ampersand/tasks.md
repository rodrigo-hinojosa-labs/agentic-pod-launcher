...
- [X] T001 Registrar la línea base en ambas versiones: correr `bats tests/` y
...
- [X] T002 En `tests/render.bats`, agregar los 10 casos oráculo del contrato §3 (A1-A10)
...
- [X] T003 Implementar `_render_replace_all TEXT PLACEHOLDER VALUE` en
...
- [X] T004 Confirmar **VERDE** de T002 en las dos versiones de bash, y
...
- [X] T005 [P] [US1] En `tests/render.bats`, agregar E2 end-to-end: un `agent.yml` con
...
- [X] T006 [P] [US1] Agregar E6: la variante **mayúscula** (`{{NAME}}`, `render.sh:95`) con
...
- [X] T007 [P] [US1] Agregar E5, no-regresión: un `agent.yml` **sin ningún `&`** produce
...
- [X] T008 [US1] Confirmar que T005-T007 están **ROJOS** bajo bash ≥5.2 (salvo T007, que
      debe estar verde antes y después — su valor es demostrar que no cambia).

### Implementación

- [X] T009 [US1] Reemplazar el call site de minúsculas, `scripts/lib/render.sh:90`, por
      `row_expanded=$(_render_replace_all "$row_expanded" "{{${field}}}" "$fval")`.
- [X] T010 [US1] Reemplazar el call site de mayúsculas, `scripts/lib/render.sh:95`, por la
      forma equivalente con `$field_upper` / `$fval_upper`. **Va en la misma entrega que
      T009**, nunca después.
- [X] T011 [US1] **NO tocar** `scripts/lib/render.sh:105-110` (la sustitución del bloque
...
- [X] T012 [US1] Confirmar **VERDE** de T005-T007 y del test heredado E4
...
- [X] T013 [US2] Agregar en `tests/render.bats` el test dedicado del contrato §5, con un
...
- [X] T014 [US2] **A4 merece su propia aserción**: hoy en bash ≥5.2 un `\&` escrito por el
      operador pierde el backslash (`ref=&!`). El contrato exige devolverlo tal cual. Ese
      caso no lo cubre ningún test existente.
- [X] T015 [US2] Mutación: revertir **un** call site a `${var//…}`, correr la suite bajo
...
- [X] T016 [P] [US3] Corregir la afirmación de `CLAUDE.md:11`: hoy dice que no hay piso de
...
- [X] T017 [P] [US3] Corregir la misma afirmación en `README.md:20`.
- [X] T018 [US3] Agregar el guard de no-drift E7 en `tests/render.bats` (o donde vivan los
...
- [X] T019 [US3] Hacer que la corrida de la suite exponga la versión del intérprete
...
- [X] T020 Suite completa **en las dos versiones**, cero fallas en ambas (SC-004). Anotar
      los dos conteos y las dos versiones exactas en el PR.
- [X] T021 `shellcheck -S error setup.sh scripts/agentctl scripts/lib/*.sh` limpio.
- [X] T022 Gate G3 del quickstart: `--regenerate` sobre un `agent.yml` sin `&` produce
      artefactos byte-idénticos a los previos al cambio (`diff` vacío de `.mcp.json` y
      `.env.example`).
- [X] T023 `CHANGELOG.md` + `VERSION`. **RESUELTO al rebasar (2026-07-19)**: 022 entró
      primero (PR #80, merge `ab4bb32` en `main`, se llevó `0.14.0`), así que esta rama
      rebasó sobre `ab4bb32` y usa **`0.15.0`**, tal como preveía esta tarea. Ojo con la
      trampa que esto destapó: los dos lados habían escrito `0.14.0`, así que **git
      auto-mergea `VERSION` sin marcar conflicto** y deja dos features reclamando la misma
      versión. El conflicto es semántico, no textual — hay que verificar `VERSION` contra
      `origin/main` a mano después de todo rebase, porque el rebase no lo va a delatar.
