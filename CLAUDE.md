...
**The INITIAL boot is not the watchdog's job** (036, v0.26.0). `main()` calls `start_initial_session`, which retries up to `MAX_BOOT_ATTEMPTS` (3) before exiting non-zero — a first boot that lost the channel-health race used to exit the container outright, and `unless-stopped` turned that single loss into a restart loop with a colder cache each time. It logs `initial session attempt N/3` per attempt and keeps the pre-036 `ERROR: initial tmux session failed to start` as a prefix of its exhaustion line, so old log scraping still matches. It also writes `$WATCHDOG_RUNTIME_DIR/boot-attempt` (`<attempt> <epoch>`) for the duration, which `agentctl doctor` reads *through `docker exec`* — `/tmp` is a container tmpfs, reachable through neither bind-mount — to tell a legitimate retry from a dead container, with a freshness guard so a permanently broken agent cannot read as "starting" forever. **Two guards in that function are load-bearing**: the `mkdir -p` (the runtime dir does not exist on attempt 1 — `/tmp` is emptied on every container start and the only other `mkdir` is *inside* `start_session`) and the `|| true` on every marker write. Without either, `set -euo pipefail` turns the diagnostic into the restart loop it exists to prevent. `_run_watchdog` itself is frozen by a sha oracle in `tests/start-services-watchdog.bats`; no test exercised it before 036, and `ebfe35f` is what happens when that area is refactored blind.
...
After a `--channels` launch the watchdog first waits for `bun server.ts` to appear before marking the session healthy (`verify_channel_healthy`). Since `026-channel-watchdog-timeout` (v0.17.0) that wait is **not** hardcoded: it defaults to 60s, resolved at call time by `channel_health_timeout()` (absent/invalid → 60). **Since `036` (v0.26.0) the source of that value is `docker.channel_health_timeout_s` in `agent.yml`**, rendered into the compose `environment:` block — which *outranks* `env_file`, so the rendered value now wins over a `CHANNEL_HEALTH_TIMEOUT` an operator had hand-written into the workspace `.env`. The in-container reader is unchanged and still consumes the same env var; only its source moved. `--regenerate` migrates a leftover `.env` value into `agent.yml` rather than resetting it (a flat 60 would have halved the window of every agent that followed the old docs — `donna` runs at 120) and prints a one-line note naming the key, never its value. The old hardcoded 20s flapped ferrari at boot (bun took ~22-25s under MCP contention). **The timeout interacts with the crash budget**: a sustained failure cycle costs ~T+5s, so 5 failures only fit the 300s window while T stays under ~70s; the default 60 is safe, and `warn_if_channel_timeout_risky` logs a boot WARN once for an override `≥65s` (no cap — the operator's value is used as-is). Docker-only; `start_services.sh` is image-baked, not mirrored to `scripts/lib/`.
...
- **`PRODUCER | grep -q` under `pipefail` is a race, not an idiom.** `grep -q` exits the instant it matches; if the producer writes again it takes EPIPE and dies with **141**, and `pipefail` hands that 141 to the caller. Inside `if PIPELINE; then` errexit does *not* fire but the condition silently goes FALSE (a wrong decision); in a plain `v=$(PIPELINE)` assignment it *aborts the script* (`local v=$(…)` masks it — the `local` builtin's own status wins — so the two shapes differ). This shipped for months in both guard backfills (`setup.sh`, fixed 0.25.1 by `agent_yml_has_plugin`, which reads the producer into a variable and decides with `case` — no second process left to signal). The **deciding factor is the producer's write pattern**, not the pipeline text: a builtin like `printf` emits its whole payload in one `write()` that the kernel accepts before the reader even runs, so it is immune **below 64 KiB** and reliably fatal above it; an external line-buffered writer (`yq`, `journalctl`, `tail`) is exposed as soon as one line follows the match. A 26-site audit (2026-09-21, all measured) found 4 real instances; these six are safe only by accident and are the ones to re-check when their inputs change:
  - `docker/scripts/start_services.sh:431` — survives *only* on the `|| true` at its call site (`:704`). Measured without it: 131/300 abort. Don't "clean up" that guard.
  - `docker/scripts/start_services.sh:596`/`:645` — survive because the `claude` CLI is Node, and Node ignores SIGPIPE (measured: 30/30 exit 0 on a forced EPIPE). That is vendor behaviour, not ours.
  - `docker/scripts/start_services.sh:1021` — survives on log-window size: cliff measured at ~64 KiB, largest real 50-line window 15,684 B. Re-measure if pane width or line shapes change.
  - `setup.sh:1544` — survives because `gh` prints ~206 B. Past 64 KiB it would `exit 1` on the benign "fork already exists" path.
  - `scripts/lib/qmd_index.sh:498` — survives because the matched string is the last thing qmd writes. With 20 bytes after the match it already fires 10.7%.
- **A big payload must never reach a test's child process through `argv` or the environment.** Linux caps each *individual* `argv`/`envp` string at `MAX_ARG_STRLEN` — 32 pages, **128 KiB** — and returns `E2BIG` ("Argument list too long"); macOS enforces only a 1 MiB *total* with no per-string limit. So `PAD="$big" bash probe.sh` is green on every local run and red on the ubuntu CI arm, which is precisely how `tests/sigpipe-boot-paths.bats` shipped broken in 0.26.0 while measuring 1502/0 twice on this Mac. Write the payload to a file and let the child read it (`journal="$(cat "$padfile")"` — command substitution happens in-process, with no exec involved). The same trap applies to `run bash -c "…" _ "$output"` when `$output` is large. **The dual-arm matrix is the only thing that sees this** — feature 025 built it for the bash-3.2-vs-5.x split, and it turns out to catch the kernel-limit split too, so a local double-green is not a gate. `tests/` is excluded from the shellcheck job, so no static check will ever flag it either.
