...
Prefer a reproducible, hand-written definition over the wizard — for scripted provisioning or standing up a second agent alongside an existing one? [`docs/creating-an-agent.md`](docs/creating-an-agent.md) walks the **declarative** method (author `agent.yml` + persona + `.env`, then `./setup.sh --non-interactive`) end-to-end, with a worked `john-doe` example and the docker-mode gotchas.
...
#### MCP startup-handshake window

Claude Code marks an MCP server **failed for the rest of the session — with no retry** if it doesn't finish its startup handshake within a fixed window (30 s by default). An MCP whose first launch downloads its package (a cold PyPI/npm cache after a rebuild or on a fresh workspace) can exceed that and stay dead until the whole session restarts — the failure that left `donna`'s `google-workspace` MCP silently dead in production. `claude.mcp_timeout_ms` in `agent.yml` sets the window (default **120000** ms, ~2.4× the measured cold download). It is single-sourced and rendered to both modes — the docker `docker-compose.yml` `environment:` block (`MCP_TIMEOUT`, like `TZ`) and the local `.state/remote-control.env` — so it survives `--regenerate`. Raise it for a slow host or a large first-time download; an absent or invalid value (non-numeric, `0`, negative) falls back to 120000. No image rebuild is needed for a change to take effect — the value is a runtime env var delivered through the compose environment, not baked into the image. To tell a genuinely-broken MCP from a merely-slow one, launch it by hand (`docker exec -u agent <container> <the command from .mcp.json>`): if it fails there too, the window is not the cause.

#### MCP warm cache

MCPs that ship as `uvx` (Python) or `npx` (Node) packages download on first use. If that download runs inside Claude Code's MCP startup-handshake window and takes too long, the server is marked failed **for the rest of the session** with no retry. To prevent that, the agent pre-warms the package cache **before Claude starts its MCPs**:

- **What is warmed** is derived from the effective `.mcp.json` — every `uvx`/`npx` server, catalog *and* overlay-injected (e.g. a `google-workspace` MCP added by an external overlay). There is no hardcoded per-MCP list; the derivation (`scripts/lib/mcp_warm.sh`) reads each server's `command` + `args`, so a wrapper command like `command=seed-creds.sh, args=[uvx, workspace-mcp]` is covered too.
- **Docker** warms both `uvx` and `npx` packages into `/opt/uv` and `/opt/npm-cache` (kept outside `/home/agent`, off the state bind-mount) at container boot. **Local** warms the `uvx` packages during `./setup.sh --login`. The step is fail-soft: a package that can't warm just degrades to the (configurable) handshake window; it never blocks startup, and it reads no secrets.

**Diagnosing a warm miss vs a broken MCP.** If an MCP is unhealthy, first tell "slow but sane" from "genuinely broken": arrange a shell in the container (`./scripts/agentctl attach` or `docker exec -u agent …`) and start the server by hand — e.g. `UV_OFFLINE=1 uvx <package> --help` for a uvx MCP, or check `uv tool list` / `ls /opt/npm-cache`. If it starts in a few seconds from the warm cache, the MCP is fine and any earlier failure was a cold-download race (raise the handshake window). If it still fails offline, the package genuinely never warmed — check the boot log for the `mcp_warm: warn: <pkg> failed` trace.
...
The agent ships with the `claude-plugins-official/telegram` channel plugin enabled by default. Boot-time post-install hooks layer seven behaviors on top of the upstream plugin:

- **Persistent "typing…" indicator** — the upstream plugin fires `sendChatAction` once per inbound and Telegram auto-expires the action at ~5 seconds. The post-install hook adds a 4-second refresh interval so the user sees "typing…" continuously while Claude is processing, including during long tool calls. The refresh has a hard cap (default 5 minutes, tunable via the `TELEGRAM_TYPING_MAX_MS` env var): on expiry it stops, sends a user-visible timeout warning to the chat, and logs the abort to stderr — instead of showing "typing…" forever. As of patch **v6** that warning names four possible causes without asserting a single one: a slow turn still running, an answer produced **without calling the reply tool**, an expired login, or a turn **blocked in a console-only interactive prompt the channel can't answer** — and points to `agentctl doctor` (v4 wrongly blamed OAuth alone, sending the operator down the wrong path).
...
- **Reply-delivery guard** — under a long conversation a live, healthy agent can answer as plain text in its own terminal instead of calling the `plugin:telegram:telegram` reply tool, so the reply never leaves the container (measured in production). The plugin records a `pending-reply.json` marker on each inbound and deletes it when the reply tool fires; a Claude Code **Stop hook** (`scripts/hooks/stop-redeliver.sh`, registered in `settings.json` at boot) fires at turn-end and, if a channel turn ended with that marker still present, re-injects one corrective nudge so the agent resends through the tool. Bounded to one attempt by default (`features.reply_guard.max_attempts`), fail-silent, and logged once to the stderr file. Enabled by default whenever the Telegram plugin is present; toggle via `features.reply_guard.enabled` in `agent.yml`.
- **AskUserQuestion guard** — the interactive selection tool renders a menu in the agent's own container terminal, a surface a Telegram operator can never see or answer; without a guard the turn hangs invisibly. A Claude Code **PreToolUse hook** (`scripts/hooks/askq-guard.sh`, matcher `AskUserQuestion`, registered in `settings.json` at boot) fires *before* the prompt opens: if the reply-delivery guard's `pending-reply.json` marker is present (a channel turn, still unreplied), it denies the tool and redirects the agent to ask the same question(s) as plain text via `plugin:telegram:telegram` instead — the operator answers in chat. Absent that marker (a console/local turn) the guard does nothing, deliberately **failing open** so a legitimate console prompt is never blocked. Bounded by `features.askuserquestion_guard.max_attempts` (default 1); past the cap the hook keeps denying — the console-only prompt still never opens — but switches to a terminal "stop retrying" reason, and one explicit give-up message is delivered to the chat through the plugin's own send path (checked on every typing-indicator tick and at the start of the next turn), so the operator is never left in silence. Enabled by default whenever the Telegram plugin is present; toggle via `features.askuserquestion_guard.enabled` in `agent.yml`. Diagnose via the same `telegram-mcp-stderr.log` (`askq-guard: redirected …` / `askq-guard: gave up …`).
- **Round-trip voice** — send a Telegram voice note instead of typing, and the agent hears it and can speak its answer back as a native voice bubble alongside the full text reply. Inbound: a DM-only, authorization-gated pipeline transcribes the note via ElevenLabs STT (Scribe v2) **detached** from the channel's sequential update loop, so a 30-second transcription never blocks other chats, commands, or permission prompts; any failure (disabled, unauthorized, over-cap, transport error) falls back to today's exact placeholder text. Outbound: the `reply` tool synthesizes a spoken rendition — the agent's own `voice_text` when provided, else a truncated version of the reply text — and sends it as a native voice bubble; a synthesis failure never breaks the text reply. **The tool's acknowledgement now tells the agent what its own voice did**, in the same turn: a `voice: sent (fmt=…, chars=…, ms=…)` or `voice: failed (step=synth|send, cls=…, status=…)` line, absent entirely when the voice step didn't run. This closes a measured defect where an agent claimed "I can't send audio" in the same turn its plugin sent a working voice bubble — the fix is the acknowledgement carrying evidence, not more prompting. **A typed message can also trigger a voice reply**, exactly like a voice note: send one of the fixed request phrases below (case- and accent-insensitive) in a direct chat, or the agent can set `voice_force: true` on its reply for wording the table doesn't cover. **The spoken rendition is a summarized narration, not the text read aloud** (034): the channel's contract asks the agent for a `voice_text` SUMMARY of about 30 seconds (~450 characters), stretching to ~45 s or at most ~60 s only when the amount of information warrants it, as plain spoken prose in the agent's configured language (`user.language`; `mixed` mirrors the user), with figures and dates in words and every amount followed by its currency name. Whatever reaches the synthesizer — `voice_text` or, when omitted, the reply text — is first made spoken-safe deterministically in the plugin: markdown, code, lists, block quotes, tables, emojis and URLs are stripped, `$`/`CLP`/`US$`/`USD`/`UF`/`%` next to a figure are spoken as words (`$1.234.567` → "1.234.567 pesos chilenos"; the currency word is `features.voice.currency`, default `pesos chilenos` / `Chilean pesos`; digits are never rewritten), the configured language is passed to the TTS as `language_code`, and an over-long rendition is cut at a **sentence** boundary at the spoken cap (`TELEGRAM_VOICE_SPOKEN_CHAR_CAP`, default now **900** characters ≈ 60 s) — the acknowledgement then adds `; voice_text trimmed to N chars`. **Every audio ends with a configured closing phrase, exactly once**: `features.voice.signoff` in `agent.yml` (default `Eso es toda la información. Cambio y fuera, {nickname}.`, or `That is all the information. Over and out, {nickname}.` for `user.language: en`; `{nickname}` is substituted from `user.nickname` at render time, sanitized, and delivered as `TELEGRAM_VOICE_SIGNOFF`), appended by the channel to both paths and deduplicated if the agent wrote it itself. Neither style field is a wizard prompt — edit them in `agent.yml` and `--regenerate`; existing workspaces are backfilled with the localized defaults. Details, mutation checks and the live gate: `specs/034-voice-spoken-style/quickstart.md`.

  | Spanish | English |
  | --- | --- |
  | responde con audio · respondeme con audio | reply with audio · respond with audio |
  | responde en/por audio · respondeme en/por audio | answer with audio · reply/respond with voice |
  | contesta con/en/por audio · contestame con/en/por audio | answer with voice · reply/respond in audio |
  | responde/contesta con voz · respondeme con voz | answer in audio |
  | responde con una nota de voz | send me an audio / a voice note / a voice message |
  | mandame un audio · mandame audio · mandame una nota de voz | send a voice note |
  | enviame un audio · enviame una nota de voz | reply with a voice note / a voice message |
  | responde hablando · respondeme hablando | |

  Omitting a spoken rendition (`voice_text`) on a long reply is recorded on every occurrence in `telegram-mcp-stderr.log`, and above a threshold derived from the spoken-length cap the acknowledgement names it too — no separate config, the threshold moves with the cap. In reply mode `always`, a failed voice attempt sets a one-shot cooldown so the agent's own "the audio didn't go out" follow-up can't itself fail and re-trigger; the next user message tries voice again normally. **Opt-in, disabled by default** — unlike the guards above, this spends money per message and needs a new secret (`ELEVENLABS_API_KEY` in `.env`); enable via `features.voice.enabled` in `agent.yml` (or answer "yes" to the wizard's voice prompt, docker mode only) and see [`specs/033-voice-reply-feedback/quickstart.md`](specs/033-voice-reply-feedback/quickstart.md) for enablement and validation steps. Cost: roughly $0.03 per round trip. Diagnose via the same `telegram-mcp-stderr.log` (`voice stt ok/fail …` / `voice tts ok/fail/spoke …` / `voice request detected …` / `voice skip: cooldown …`).

All seven hooks are idempotent (each guarded by a marker comment) and fail-silent if any anchor in the upstream plugin source drifts — the plugin keeps its default behavior in that case.

Separately, the supervisor guards the channel's **boot health check**: after a `--channels` launch it waits for the plugin's `bun server.ts` to come up before marking the session healthy, and kills + respawns the session if it never appears. That wait defaults to **60 seconds** and is set with `docker.channel_health_timeout_s` (seconds) in `agent.yml` — raise it on a slow host or one running many MCPs, where the plugin can take longer to start under boot contention (a hardcoded 20s used to flap such hosts in a permanent restart loop). An absent or invalid value falls back to 60; a value of `65` or higher logs a boot warning that the container's crash budget may no longer escalate a genuinely-dead channel to a restart. The container still reads the `CHANNEL_HEALTH_TIMEOUT` env var — only its *source* moved: `--regenerate` renders the field into the compose `environment:` block, which outranks `env_file:`. If you had set the key by hand in the workspace `.env`, the next `--regenerate` migrates that value into `agent.yml`, prints a one-line note, and you can then delete the `.env` line.
